# EasyMobs RPG Integration Example

This directory contains a minimal, runnable Paper plugin that shows how an **external plugin** can customize every part of EasyMobs through the public `EasyMobsAPI` — no YAML files inside `plugins/EasyMobs/` required.

## What it demonstrates

| EasyMobs feature | API used | What the example does |
|---|---|---|
...
1. Build EasyMobs first so the local JAR exists:
...
`EasyMobsRpgIntegration` declares `depend: [EasyMobs]`, so Paper will load EasyMobs first.
...
`EasyMobsAPI#getInstance()` reads the service registered by EasyMobs through Bukkit's `ServicesManager`.
...
The keys and value formats are the same ones used in EasyMobs YAML configs (items, mobs, skills, spawn rules).

## Custom skill effects

To add an effect type EasyMobs does not know about, implement the public `SkillEffect` interface and register a factory:
...
- The example depends on `target/easymobs-1.0.0-SNAPSHOT.jar` via a Maven `system` scope. In a real project you would install EasyMobs into your local/remote Maven repository or use a multi-module build.
