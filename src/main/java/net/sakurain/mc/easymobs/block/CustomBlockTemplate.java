package net.sakurain.mc.easymobs.block;
...
public final class CustomBlockTemplate {
    ...
    public CustomBlockTemplate(@NotNull EasyMobsPlugin plugin, @NotNull String id, @NotNull ConfigurationSection section) {
        this.plugin = plugin;
    ...
    public boolean canBreak(@Nullable Player player, @Nullable ItemStack tool) {
        ...
        for (String allowed : allowedBreakItems) {
            ...
            if (allowed.startsWith("ezmobs:") && allowed.substring(7).equalsIgnoreCase(itemId)) {
                return true;
            }
